...
function buildHello({
  ...
  const advertisedCapabilities = Array.isArray(capabilities) ? capabilities.filter(value => !['lanpet-v1', 'lanpet-world-v1', 'lanpet-dice-v1'].includes(value)) : []
  if (lanpetSharingEnabled) advertisedCapabilities.push('lanpet-v1', 'lanpet-world-v1', 'lanpet-dice-v1')
  return {
    type: 'hello',
    v: WIRE_VERSION,
    fromId: peerId,
...
// (제거됨 #69) adaptV1KeyExchangeToHello — v1 key-exchange → v2 hello 어댑터.
// v1 수신 핸들러 제거와 함께 삭제. 현재 앱(v0.8.0+)은 항상 v2 hello 를 보낸다.
