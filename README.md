...
채팅과 함께 돌보는 독자 가상 펫입니다. 상단의 **Lanpet** 버튼에서 시작하며, 펫 생성과 다른 피어에게 공개하는 선택을 분리했습니다.

- 평일 **09:00–18:00**에 돌봄·성장·친구 활동을 할 수 있습니다. 밤과 주말은 무벌점 휴식입니다.
- 돌봄과 놀이, 선택형 3단계 성장, 친구 방문·협동 놀이·기념품·5턴 친선 배틀을 제공합니다.
- 배틀은 양측 수락 후 동등한 조건으로 진행하며 순위·약탈·연승 보상이 없습니다.
- 펫 정보와 교류 기록은 로컬 암호화 DB에 저장합니다. 채팅 본문·메시지 수를 성장에 사용하지 않고, 펫 OS 알림도 보내지 않습니다.
- 활동별 수신 허용과 피어 초대 차단을 설정할 수 있습니다. 결과 전달이 끊기면 확인 대기 상태로 표시하고 재연결 때 복구합니다.
- 교류하려면 **양쪽 앱이 v0.13.0 이상**이고 공개가 켜져 있어야 합니다. 구버전과의 기존 채팅은 유지됩니다.

상세 제품·통신 설계와 확정 사항은 [설계 문서](docs/lan-chat-pet-feature-design.md)에 있습니다.
...
macOS 배포는 Developer ID 서명과 공증을 구분해 확인합니다. 기존 공증 프로필을 로컬에서 준비하고 `APPLE_KEYCHAIN_PROFILE`을 지정한 후 다음 순서로 실행합니다. 계정 암호나 토큰을 채팅이나 저장소에 입력하지 않습니다.

```bash
npm run release:package  # 테스트·빌드·서명·공증, 게시 없음
npm run release:verify   # codesign·Gatekeeper·공증 티켓·업데이트 SHA-512 검증
```

검증을 통과한 버전의 앱·ZIP·DMG·업데이트 메타데이터만 함께 게시합니다. 공증 자격 증명이 없거나 검증이 실패하면 배포 완료로 처리하지 않습니다.
